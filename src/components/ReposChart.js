...
const ReposChart = () => {
    ...
    const {user , loading , repo , stats} = React.useContext(ContextData)

    const countOccurrences = (arr, val) => arr.reduce((a, v) => (v === val ? a + 1 : a), 0);
    
    let statArray = [] 

    let show = stats.forEach(function(each){            //  push all langauges into setArray
    ...
    for (const [key, value] of Object.entries(newSet)) {    // pushes values into newSetSet  
